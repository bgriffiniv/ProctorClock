...
class Clock:
  ...
  #
  def __init__(self, root, delta_hours=0, is_show_image=False, width=400, height=400, is_use_thread=False):
    self.world = [-1, -1, 1, 1]
    ...
    self.circlesize = 0.1 # defines the radius of the center circle as 0.09 ??
  ...
  #
  def seek_time(self, event, hour_change=0, minute_change=0):
  ...
  def add_minute(self, event):
    if event:
      self.seek_time(event, minute_change=1)
  ...
  def paint_number(self, x_coord, y_coord, i):
    ...
    sct = self.canvas.create_text
    x, y = self.my_mapper._window_to_viewport(x_coord, y_coord)
    sct(x, y, font='Helvetica 24 bold', text=i)
